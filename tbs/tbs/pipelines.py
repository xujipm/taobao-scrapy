...
class TbsPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        if item['page'] == 1:
            ...
            # sqlAll = sqlAll + ";\n" + "UPDATE `taobao_items` SET " +\
            #     "`updating`='1' where " +\
            #     "`item_id` ='" + item['itemId'] + "'"
            # cursor.execute(sql)
            # conn.commit()
            # 将任务放进redis中,min()的作用最多新抓取200页
            for pageNum in range(2, min(data['total'] // 20 + 2, 20)):
        ...
        # 分条存储评价
        pingjia = ""
        sql = "\n" + "INSERT INTO `pingjia` (`item_id`,`rate_id`,`content`," +\
        ...
        if cursor.execute("select * from `taobao_items` where `item_id` = " +
            ...
            sqlAll = sqlAll + ";\n" + "INSERT INTO `taobao_items` " +\
                "(`item_id`, " + "`pingjia_1`," + \
                "`item_pingjiashu`,`last_update_time`," +\
                " `update_times`) VALUES ('" + item['itemId'] + "','" +\
                pingjia1 + "','" + str(data['total']) + "',NOW(),'1')"
        # print(sql)
        # cursor.execute(sql)
        # conn.commit()
